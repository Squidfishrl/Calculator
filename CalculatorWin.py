...
root = tkinter.Tk()
screenWidth = dw.availableGeometry().width()
screenHeight = dw.availableGeometry().height()
del app, dw
root.title("Calculator")
root.geometry("%sx67+0+%s" % (screenWidth, screenHeight-25))
...
class CalculatorButtonMenu:
    ...
    def popUpMoreFuncFrame(self, main):
        if calc.numberAndFuncLayoutFrame.winfo_ismapped():
            ...
            main.geometry("%sx67+0+%s" % (screenWidth, screenHeight-25))
            ...
            main.geometry("%sx98+0+%s" % (screenWidth, screenHeight-50))
...
class CalculatorNumberAndFunctionLayout:
    ...
    def __init__(self, frame):
        ...
        self.btnpi = tkinter.Button(frame, text="pi", command=lambda: self.insertInUserInputEntry('math.pi'))
        ...
        self.btne = tkinter.Button(frame, text="tau", command=lambda: self.insertInUserInputEntry('math.tau'))
        self.btne.pack(side="left")
